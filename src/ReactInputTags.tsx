...
export const ReactInputTags: FC<ReactInputTagsProps> = ({
  ...
  const uniqueOptions = options
    ? options.filter((option, index, self) => self.findIndex((item) => item.label === option.label) === index)
    : [];
  ...
  const handleSelectOption = ({ label, value }: TagOptions) => {
    const isSelected = tags.some((tag) => tag.value === value);
    let newTags;
    const tag = { value, label };

    if (isSelected) {
      // If selected, remove it from tags
      newTags = tags.filter((tag) => tag.value !== value);
    } else {
      // If not selected, add it to tags
      newTags = [...tags, tag];
    }
  ...
  const handleKeyDown = (e: KeyboardEvent<HTMLDivElement>) => {
    ...
    if (e.key === "ArrowDown") {
      ...
      if (showOptions && highlightedIndex !== -1 && highlightedIndex < filteredOptions.length) {
  ...
  const scrollOptionIntoView = (index: number) => {
    if (optionContainerRef.current) {
      const optionElement = optionContainerRef.current.querySelector(
        `.react-select-tags-select-container-option:nth-child(${index + 1})`
      );
      if (optionElement) {
  ...
  return (
    <div className="react-select-tags-main" ref={selectRef} onKeyDown={handleKeyDown} data-test="main-container">
      <div className={`${style?.mainContainer} react-select-tags`} onClick={handleClick} data-test="react-select-tags">
        ...
        {renderInput && (
          <input
            value={inputValue}
            ref={inputRef}
            className={`${style?.input} react-select-tags-input`}
      ...
      {showOptions && filteredOptions && filteredOptions?.length > 0 && (
        <div
          className={`${style?.optionContainer} react-select-tags-select-container`}
            ...
            return (
              <div
                ...
                className={`${style?.option} 
                  ${highlightedIndex === index ? `highlighted-option ${style?.selectedOption}` : ""} 
                  ${isSelected ? "selected-option" : ""} 
                  react-select-tags-select-container-option`}
